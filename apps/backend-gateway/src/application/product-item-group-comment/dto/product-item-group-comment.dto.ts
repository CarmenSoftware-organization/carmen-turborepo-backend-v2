...
export const CreateProductItemGroupCommentSchema = z.object({ product_item_group_id: z.string().uuid(), message: z.string().optional().nullable(), type: z.enum(['user', 'system']).default('user'), attachments: z.array(AttachmentSchema).optional().default([]) });

export class CreateProductItemGroupCommentDto extends createZodDto(CreateProductItemGroupCommentSchema) {
  @ApiProperty({ description: 'The ID of the product-item-group', example: '123e4567-e89b-12d3-a456-426614174000' }) product_item_group_id: string;
...
export const UpdateProductItemGroupCommentSchema = z.object({ message: z.string().optional().nullable(), attachments: z.array(AttachmentSchema).optional() });
