...
@Injectable()
export class ProductItemGroupCommentService {
  private readonly logger: BackendLogger = new BackendLogger(ProductItemGroupCommentService.name);
  ...
  async findById(id: string, user_id: string, bu_code: string, version: string): Promise<unknown> {
    const res: Observable<MicroserviceResponse> = this.businessService.send(
      { cmd: 'product-item-group-comment.find-by-id', service: 'product-item-group-comment' },
  ...
  async findAllByProductItemGroupId(product_item_group_id: string, user_id: string, bu_code: string, paginate: IPaginate, version: string): Promise<unknown> {
    const res: Observable<MicroserviceResponse> = this.businessService.send(
      { cmd: 'product-item-group-comment.find-all-by-product-item-group-id', service: 'product-item-group-comment' },
      { product_item_group_id, user_id, bu_code, paginate, version, ...getGatewayRequestContext() },
    );
    const response = await firstValueFrom(res);
    if (response.response.status !== HttpStatus.OK) return Result.error(response.response.message, httpStatusToErrorCode(response.response.status));
  ...
  async create(data: Record<string, unknown>, user_id: string, bu_code: string, version: string): Promise<unknown> {
    const res: Observable<MicroserviceResponse> = this.businessService.send(
      { cmd: 'product-item-group-comment.create', service: 'product-item-group-comment' },
  ...
  async update(id: string, data: Record<string, unknown>, user_id: string, bu_code: string, version: string): Promise<unknown> {
    const res: Observable<MicroserviceResponse> = this.businessService.send(
      { cmd: 'product-item-group-comment.update', service: 'product-item-group-comment' },
  ...
  async delete(id: string, user_id: string, bu_code: string, version: string): Promise<unknown> {
    const res: Observable<MicroserviceResponse> = this.businessService.send(
      { cmd: 'product-item-group-comment.delete', service: 'product-item-group-comment' },
      { id, user_id, bu_code, version, ...getGatewayRequestContext() },
    );
    const response = await firstValueFrom(res);
    if (response.response.status !== HttpStatus.OK) return Result.error(response.response.message, httpStatusToErrorCode(response.response.status));
  ...
  async addAttachment(id: string, attachment: Record<string, unknown>, user_id: string, bu_code: string, version: string): Promise<unknown> {
    const res: Observable<MicroserviceResponse> = this.businessService.send(
      { cmd: 'product-item-group-comment.add-attachment', service: 'product-item-group-comment' },
  ...
  async removeAttachment(id: string, fileToken: string, user_id: string, bu_code: string, version: string): Promise<unknown> {
    const res: Observable<MicroserviceResponse> = this.businessService.send(
      { cmd: 'product-item-group-comment.remove-attachment', service: 'product-item-group-comment' },
        ...
        this.logger.warn(
          {
            ...
            bu_code,
            product_item_group_id: dto.product_item_group_id,
            uploaded_count: uploaded.length,
            failed_count: failures.length,
          },
          ProductItemGroupCommentService.name,
        );
        await Promise.all(
