...
export const CreateProductCommentSchema = z.object({ product_id: z.string().uuid(), message: z.string().optional().nullable(), type: z.enum(['user', 'system']).default('user'), attachments: z.array(AttachmentSchema).optional().default([]) });

export class CreateProductCommentDto extends createZodDto(CreateProductCommentSchema) {
  @ApiProperty({ description: 'The ID of the product', example: '123e4567-e89b-12d3-a456-426614174000' }) product_id: string;
...
export const UpdateProductCommentSchema = z.object({ message: z.string().optional().nullable(), attachments: z.array(AttachmentSchema).optional() });
