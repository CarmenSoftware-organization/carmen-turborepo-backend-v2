...
export class UnitCommentController {
  ...
  @Post(':bu_code/unit-comment/upload')
  @UseGuards(new AppIdGuard('unitComment.createWithFiles'))
  @UseInterceptors(FilesInterceptor('files'))
  @ApiVersionMinRequest()
  @ApiOperation({
    summary: 'Create a unit comment with file uploads',
    operationId: 'createUnitCommentWithFiles',
    responses: {
  ...
  async createWithFiles(
    ...
    const { user_id } = ExtractRequestHeader(req);
    return this.unitCommentService.createWithFiles(
      files,
      body,
      user_id,
      bu_code,
