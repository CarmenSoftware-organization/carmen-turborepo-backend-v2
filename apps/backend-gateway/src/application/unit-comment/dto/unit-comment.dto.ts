...
export const CreateUnitCommentSchema = z.object({ unit_id: z.string().uuid(), message: z.string().optional().nullable(), type: z.enum(['user', 'system']).default('user'), attachments: z.array(AttachmentSchema).optional().default([]) });

export class CreateUnitCommentDto extends createZodDto(CreateUnitCommentSchema) {
  @ApiProperty({ description: 'The ID of the unit', example: '123e4567-e89b-12d3-a456-426614174000' }) unit_id: string;
...
export const UpdateUnitCommentSchema = z.object({ message: z.string().optional().nullable(), attachments: z.array(AttachmentSchema).optional() });
