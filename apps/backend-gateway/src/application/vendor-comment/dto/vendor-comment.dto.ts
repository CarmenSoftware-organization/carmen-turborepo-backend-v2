...
export const CreateVendorCommentSchema = z.object({ vendor_id: z.string().uuid(), message: z.string().optional().nullable(), type: z.enum(['user', 'system']).default('user'), attachments: z.array(AttachmentSchema).optional().default([]) });

export class CreateVendorCommentDto extends createZodDto(CreateVendorCommentSchema) {
  @ApiProperty({ description: 'The ID of the vendor', example: '123e4567-e89b-12d3-a456-426614174000' }) vendor_id: string;
...
export const UpdateVendorCommentSchema = z.object({ message: z.string().optional().nullable(), attachments: z.array(AttachmentSchema).optional() });
