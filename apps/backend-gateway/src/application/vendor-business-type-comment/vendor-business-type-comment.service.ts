...
export class VendorBusinessTypeCommentService {
  ...
  async findById(id: string, user_id: string, bu_code: string, version: string): Promise<unknown> {
    const res: Observable<MicroserviceResponse> = this.businessService.send(
      { cmd: 'vendor-business-type-comment.find-by-id', service: 'vendor-business-type-comment' },
  ...
  async findAllByVendorBusinessTypeId(vendor_business_type_id: string, user_id: string, bu_code: string, paginate: IPaginate, version: string): Promise<unknown> {
    const res: Observable<MicroserviceResponse> = this.businessService.send(
      { cmd: 'vendor-business-type-comment.find-all-by-vendor-business-type-id', service: 'vendor-business-type-comment' },
      { vendor_business_type_id, user_id, bu_code, paginate, version, ...getGatewayRequestContext() },
    );
    const response = await firstValueFrom(res);
    if (response.response.status !== HttpStatus.OK) return Result.error(response.response.message, httpStatusToErrorCode(response.response.status));
  ...
  async create(data: Record<string, unknown>, user_id: string, bu_code: string, version: string): Promise<unknown> {
    const res: Observable<MicroserviceResponse> = this.businessService.send(
      { cmd: 'vendor-business-type-comment.create', service: 'vendor-business-type-comment' },
  ...
  async update(id: string, data: Record<string, unknown>, user_id: string, bu_code: string, version: string): Promise<unknown> {
    const res: Observable<MicroserviceResponse> = this.businessService.send(
      { cmd: 'vendor-business-type-comment.update', service: 'vendor-business-type-comment' },
  ...
  async delete(id: string, user_id: string, bu_code: string, version: string): Promise<unknown> {
    const res: Observable<MicroserviceResponse> = this.businessService.send(
      { cmd: 'vendor-business-type-comment.delete', service: 'vendor-business-type-comment' },
      { id, user_id, bu_code, version, ...getGatewayRequestContext() },
    );
    const response = await firstValueFrom(res);
    if (response.response.status !== HttpStatus.OK) return Result.error(response.response.message, httpStatusToErrorCode(response.response.status));
  ...
  async addAttachment(id: string, attachment: Record<string, unknown>, user_id: string, bu_code: string, version: string): Promise<unknown> {
    const res: Observable<MicroserviceResponse> = this.businessService.send(
      { cmd: 'vendor-business-type-comment.add-attachment', service: 'vendor-business-type-comment' },
  ...
  async removeAttachment(id: string, fileToken: string, user_id: string, bu_code: string, version: string): Promise<unknown> {
    const res: Observable<MicroserviceResponse> = this.businessService.send(
      { cmd: 'vendor-business-type-comment.remove-attachment', service: 'vendor-business-type-comment' },
        ...
        this.logger.warn(
          {
            ...
            bu_code,
            vendor_business_type_id: dto.vendor_business_type_id,
            uploaded_count: uploaded.length,
            failed_count: failures.length,
          },
          VendorBusinessTypeCommentService.name,
        );
        await Promise.all(
