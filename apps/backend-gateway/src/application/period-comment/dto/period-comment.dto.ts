...
export const CreatePeriodCommentSchema = z.object({ period_id: z.string().uuid(), message: z.string().optional().nullable(), type: z.enum(['user', 'system']).default('user'), attachments: z.array(AttachmentSchema).optional().default([]) });

export class CreatePeriodCommentDto extends createZodDto(CreatePeriodCommentSchema) {
  @ApiProperty({ description: 'The ID of the period', example: '123e4567-e89b-12d3-a456-426614174000' }) period_id: string;
...
export const UpdatePeriodCommentSchema = z.object({ message: z.string().optional().nullable(), attachments: z.array(AttachmentSchema).optional() });
