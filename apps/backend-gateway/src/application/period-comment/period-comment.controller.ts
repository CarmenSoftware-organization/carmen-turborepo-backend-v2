...
export class PeriodCommentController {
  ...
  @Post(':bu_code/period-comment/upload')
  @UseGuards(new AppIdGuard('periodComment.createWithFiles'))
  @UseInterceptors(FilesInterceptor('files'))
  @ApiVersionMinRequest()
  @ApiOperation({
    summary: 'Create a period comment with file uploads',
    operationId: 'createPeriodCommentWithFiles',
    responses: {
  ...
  async createWithFiles(
    ...
    const { user_id } = ExtractRequestHeader(req);
    return this.periodCommentService.createWithFiles(
      files,
      body,
      user_id,
      bu_code,
