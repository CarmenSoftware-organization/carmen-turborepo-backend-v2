...
export const CreateLocationCommentSchema = z.object({ location_id: z.string().uuid(), message: z.string().optional().nullable(), type: z.enum(['user', 'system']).default('user'), attachments: z.array(AttachmentSchema).optional().default([]) });

export class CreateLocationCommentDto extends createZodDto(CreateLocationCommentSchema) {
  @ApiProperty({ description: 'The ID of the location', example: '123e4567-e89b-12d3-a456-426614174000' }) location_id: string;
...
export const UpdateLocationCommentSchema = z.object({ message: z.string().optional().nullable(), attachments: z.array(AttachmentSchema).optional() });
