...
export class PricelistTemplateDetailCommentService {
  ...
  async findById(id: string, user_id: string, bu_code: string, version: string): Promise<unknown> {
    const res: Observable<MicroserviceResponse> = this.businessService.send(
      { cmd: 'pricelist-template-detail-comment.find-by-id', service: 'pricelist-template-detail-comment' },
  ...
  async findAllByPricelistTemplateDetailId(pricelist_template_detail_id: string, user_id: string, bu_code: string, paginate: IPaginate, version: string): Promise<unknown> {
    const res: Observable<MicroserviceResponse> = this.businessService.send(
      { cmd: 'pricelist-template-detail-comment.find-all-by-pricelist-template-detail-id', service: 'pricelist-template-detail-comment' },
      { pricelist_template_detail_id, user_id, bu_code, paginate, version, ...getGatewayRequestContext() },
    );
    const response = await firstValueFrom(res);
    if (response.response.status !== HttpStatus.OK) return Result.error(response.response.message, httpStatusToErrorCode(response.response.status));
  ...
  async create(data: Record<string, unknown>, user_id: string, bu_code: string, version: string): Promise<unknown> {
    const res: Observable<MicroserviceResponse> = this.businessService.send(
      { cmd: 'pricelist-template-detail-comment.create', service: 'pricelist-template-detail-comment' },
  ...
  async update(id: string, data: Record<string, unknown>, user_id: string, bu_code: string, version: string): Promise<unknown> {
    const res: Observable<MicroserviceResponse> = this.businessService.send(
      { cmd: 'pricelist-template-detail-comment.update', service: 'pricelist-template-detail-comment' },
  ...
  async delete(id: string, user_id: string, bu_code: string, version: string): Promise<unknown> {
    const res: Observable<MicroserviceResponse> = this.businessService.send(
      { cmd: 'pricelist-template-detail-comment.delete', service: 'pricelist-template-detail-comment' },
      { id, user_id, bu_code, version, ...getGatewayRequestContext() },
    );
    const response = await firstValueFrom(res);
    if (response.response.status !== HttpStatus.OK) return Result.error(response.response.message, httpStatusToErrorCode(response.response.status));
  ...
  async addAttachment(id: string, attachment: Record<string, unknown>, user_id: string, bu_code: string, version: string): Promise<unknown> {
    const res: Observable<MicroserviceResponse> = this.businessService.send(
      { cmd: 'pricelist-template-detail-comment.add-attachment', service: 'pricelist-template-detail-comment' },
  ...
  async removeAttachment(id: string, fileToken: string, user_id: string, bu_code: string, version: string): Promise<unknown> {
    const res: Observable<MicroserviceResponse> = this.businessService.send(
      { cmd: 'pricelist-template-detail-comment.remove-attachment', service: 'pricelist-template-detail-comment' },
        ...
        this.logger.warn(
          {
            ...
            bu_code,
            pricelist_template_detail_id: dto.pricelist_template_detail_id,
            uploaded_count: uploaded.length,
            failed_count: failures.length,
          },
          PricelistTemplateDetailCommentService.name,
        );
        await Promise.all(
