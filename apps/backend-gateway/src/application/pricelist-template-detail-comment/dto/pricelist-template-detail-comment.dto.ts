...
export const CreatePricelistTemplateDetailCommentSchema = z.object({ pricelist_template_detail_id: z.string().uuid(), message: z.string().optional().nullable(), type: z.enum(['user', 'system']).default('user'), attachments: z.array(AttachmentSchema).optional().default([]) });

export class CreatePricelistTemplateDetailCommentDto extends createZodDto(CreatePricelistTemplateDetailCommentSchema) {
  @ApiProperty({ description: 'The ID of the pricelist-template-detail', example: '123e4567-e89b-12d3-a456-426614174000' }) pricelist_template_detail_id: string;
...
export const UpdatePricelistTemplateDetailCommentSchema = z.object({ message: z.string().optional().nullable(), attachments: z.array(AttachmentSchema).optional() });
