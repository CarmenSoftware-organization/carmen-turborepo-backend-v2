...
export class TransferDetailCommentController {
  ...
  async findAllByTransferDetailId(
    ...
    @Param('transfer_detail_id', new ParseUUIDPipe({ version: '4' })) transfer_detail_id: string,
    @Req() req: Request,
  ...
  @Post(':bu_code/transfer-detail-comment/upload')
  @UseGuards(new AppIdGuard('transferDetailComment.createWithFiles'))
  @UseInterceptors(FilesInterceptor('files'))
  @ApiVersionMinRequest()
  @ApiOperation({
    summary: 'Create a transfer-detail comment with file uploads',
    operationId: 'createTransferDetailCommentWithFiles',
    responses: {
  ...
  async createWithFiles(
    ...
    const { user_id } = ExtractRequestHeader(req);
    return this.transferDetailCommentService.createWithFiles(
      files,
      body,
      user_id,
      bu_code,
