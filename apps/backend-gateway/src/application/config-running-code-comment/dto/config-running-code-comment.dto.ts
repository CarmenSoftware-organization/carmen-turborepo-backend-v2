...
export const CreateConfigRunningCodeCommentSchema = z.object({ config_running_code_id: z.string().uuid(), message: z.string().optional().nullable(), type: z.enum(['user', 'system']).default('user'), attachments: z.array(AttachmentSchema).optional().default([]) });
...
export const UpdateConfigRunningCodeCommentSchema = z.object({ message: z.string().optional().nullable(), attachments: z.array(AttachmentSchema).optional() });
