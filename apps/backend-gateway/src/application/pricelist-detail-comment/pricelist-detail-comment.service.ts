...
export class PricelistDetailCommentService {
  ...
  async findById(id: string, user_id: string, bu_code: string, version: string): Promise<unknown> {
    const res: Observable<MicroserviceResponse> = this.businessService.send(
      { cmd: 'pricelist-detail-comment.find-by-id', service: 'pricelist-detail-comment' },
  ...
  async findAllByPricelistDetailId(pricelist_detail_id: string, user_id: string, bu_code: string, paginate: IPaginate, version: string): Promise<unknown> {
    const res: Observable<MicroserviceResponse> = this.businessService.send(
      { cmd: 'pricelist-detail-comment.find-all-by-pricelist-detail-id', service: 'pricelist-detail-comment' },
      { pricelist_detail_id, user_id, bu_code, paginate, version, ...getGatewayRequestContext() },
    );
    const response = await firstValueFrom(res);
    if (response.response.status !== HttpStatus.OK) return Result.error(response.response.message, httpStatusToErrorCode(response.response.status));
  ...
  async create(data: Record<string, unknown>, user_id: string, bu_code: string, version: string): Promise<unknown> {
    const res: Observable<MicroserviceResponse> = this.businessService.send(
      { cmd: 'pricelist-detail-comment.create', service: 'pricelist-detail-comment' },
  ...
  async update(id: string, data: Record<string, unknown>, user_id: string, bu_code: string, version: string): Promise<unknown> {
    const res: Observable<MicroserviceResponse> = this.businessService.send(
      { cmd: 'pricelist-detail-comment.update', service: 'pricelist-detail-comment' },
  ...
  async delete(id: string, user_id: string, bu_code: string, version: string): Promise<unknown> {
    const res: Observable<MicroserviceResponse> = this.businessService.send(
      { cmd: 'pricelist-detail-comment.delete', service: 'pricelist-detail-comment' },
      { id, user_id, bu_code, version, ...getGatewayRequestContext() },
    );
    const response = await firstValueFrom(res);
    if (response.response.status !== HttpStatus.OK) return Result.error(response.response.message, httpStatusToErrorCode(response.response.status));
  ...
  async addAttachment(id: string, attachment: Record<string, unknown>, user_id: string, bu_code: string, version: string): Promise<unknown> {
    const res: Observable<MicroserviceResponse> = this.businessService.send(
      { cmd: 'pricelist-detail-comment.add-attachment', service: 'pricelist-detail-comment' },
  ...
  async removeAttachment(id: string, fileToken: string, user_id: string, bu_code: string, version: string): Promise<unknown> {
    const res: Observable<MicroserviceResponse> = this.businessService.send(
      { cmd: 'pricelist-detail-comment.remove-attachment', service: 'pricelist-detail-comment' },
        ...
        this.logger.warn(
          {
            ...
            bu_code,
            pricelist_detail_id: dto.pricelist_detail_id,
            uploaded_count: uploaded.length,
            failed_count: failures.length,
          },
          PricelistDetailCommentService.name,
        );
        await Promise.all(
