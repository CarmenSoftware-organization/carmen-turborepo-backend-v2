...
export const CreatePricelistDetailCommentSchema = z.object({ pricelist_detail_id: z.string().uuid(), message: z.string().optional().nullable(), type: z.enum(['user', 'system']).default('user'), attachments: z.array(AttachmentSchema).optional().default([]) });

export class CreatePricelistDetailCommentDto extends createZodDto(CreatePricelistDetailCommentSchema) {
  @ApiProperty({ description: 'The ID of the pricelist-detail', example: '123e4567-e89b-12d3-a456-426614174000' }) pricelist_detail_id: string;
...
export const UpdatePricelistDetailCommentSchema = z.object({ message: z.string().optional().nullable(), attachments: z.array(AttachmentSchema).optional() });
