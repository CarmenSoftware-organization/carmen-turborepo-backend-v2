...
export const CreateCreditTermCommentSchema = z.object({ credit_term_id: z.string().uuid(), message: z.string().optional().nullable(), type: z.enum(['user', 'system']).default('user'), attachments: z.array(AttachmentSchema).optional().default([]) });

export class CreateCreditTermCommentDto extends createZodDto(CreateCreditTermCommentSchema) {
  @ApiProperty({ description: 'The ID of the credit-term', example: '123e4567-e89b-12d3-a456-426614174000' }) credit_term_id: string;
...
export const UpdateCreditTermCommentSchema = z.object({ message: z.string().optional().nullable(), attachments: z.array(AttachmentSchema).optional() });
