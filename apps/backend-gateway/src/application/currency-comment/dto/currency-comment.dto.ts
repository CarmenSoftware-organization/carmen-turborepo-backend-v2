...
export const CreateCurrencyCommentSchema = z.object({ currency_id: z.string().uuid(), message: z.string().optional().nullable(), type: z.enum(['user', 'system']).default('user'), attachments: z.array(AttachmentSchema).optional().default([]) });

export class CreateCurrencyCommentDto extends createZodDto(CreateCurrencyCommentSchema) {
  @ApiProperty({ description: 'The ID of the currency', example: '123e4567-e89b-12d3-a456-426614174000' }) currency_id: string;
...
export const UpdateCurrencyCommentSchema = z.object({ message: z.string().optional().nullable(), attachments: z.array(AttachmentSchema).optional() });
