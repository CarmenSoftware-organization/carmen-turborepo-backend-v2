...
export const CreatePricelistCommentSchema = z.object({ pricelist_id: z.string().uuid(), message: z.string().optional().nullable(), type: z.enum(['user', 'system']).default('user'), attachments: z.array(AttachmentSchema).optional().default([]) });

export class CreatePricelistCommentDto extends createZodDto(CreatePricelistCommentSchema) {
  @ApiProperty({ description: 'The ID of the pricelist', example: '123e4567-e89b-12d3-a456-426614174000' }) pricelist_id: string;
...
export const UpdatePricelistCommentSchema = z.object({ message: z.string().optional().nullable(), attachments: z.array(AttachmentSchema).optional() });
