...
export const CreateDepartmentCommentSchema = z.object({ department_id: z.string().uuid(), message: z.string().optional().nullable(), type: z.enum(['user', 'system']).default('user'), attachments: z.array(AttachmentSchema).optional().default([]) });

export class CreateDepartmentCommentDto extends createZodDto(CreateDepartmentCommentSchema) {
  @ApiProperty({ description: 'The ID of the department', example: '123e4567-e89b-12d3-a456-426614174000' }) department_id: string;
...
export const UpdateDepartmentCommentSchema = z.object({ message: z.string().optional().nullable(), attachments: z.array(AttachmentSchema).optional() });
