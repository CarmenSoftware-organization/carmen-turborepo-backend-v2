...
export class DeliveryPointCommentController {
  ...
  @Post(':bu_code/delivery-point-comment/upload')
  @UseGuards(new AppIdGuard('deliveryPointComment.createWithFiles'))
  @UseInterceptors(FilesInterceptor('files'))
  @ApiVersionMinRequest()
  @ApiOperation({
    summary: 'Create a delivery-point comment with file uploads',
    operationId: 'createDeliveryPointCommentWithFiles',
    responses: {
  ...
  async createWithFiles(
    ...
    const { user_id } = ExtractRequestHeader(req);
    return this.deliveryPointCommentService.createWithFiles(
      files,
      body,
      user_id,
      bu_code,
