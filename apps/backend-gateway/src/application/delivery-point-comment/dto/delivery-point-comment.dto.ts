...
export const CreateDeliveryPointCommentSchema = z.object({ delivery_point_id: z.string().uuid(), message: z.string().optional().nullable(), type: z.enum(['user', 'system']).default('user'), attachments: z.array(AttachmentSchema).optional().default([]) });

export class CreateDeliveryPointCommentDto extends createZodDto(CreateDeliveryPointCommentSchema) {
  @ApiProperty({ description: 'The ID of the delivery-point', example: '123e4567-e89b-12d3-a456-426614174000' }) delivery_point_id: string;
...
export const UpdateDeliveryPointCommentSchema = z.object({ message: z.string().optional().nullable(), attachments: z.array(AttachmentSchema).optional() });
