...
export const CreatePricelistTemplateCommentSchema = z.object({ pricelist_template_id: z.string().uuid(), message: z.string().optional().nullable(), type: z.enum(['user', 'system']).default('user'), attachments: z.array(AttachmentSchema).optional().default([]) });

export class CreatePricelistTemplateCommentDto extends createZodDto(CreatePricelistTemplateCommentSchema) {
  @ApiProperty({ description: 'The ID of the pricelist-template', example: '123e4567-e89b-12d3-a456-426614174000' }) pricelist_template_id: string;
...
export const UpdatePricelistTemplateCommentSchema = z.object({ message: z.string().optional().nullable(), attachments: z.array(AttachmentSchema).optional() });
