...
export const CreateProductCategoryCommentSchema = z.object({ product_category_id: z.string().uuid(), message: z.string().optional().nullable(), type: z.enum(['user', 'system']).default('user'), attachments: z.array(AttachmentSchema).optional().default([]) });

export class CreateProductCategoryCommentDto extends createZodDto(CreateProductCategoryCommentSchema) {
  @ApiProperty({ description: 'The ID of the product-category', example: '123e4567-e89b-12d3-a456-426614174000' }) product_category_id: string;
...
export const UpdateProductCategoryCommentSchema = z.object({ message: z.string().optional().nullable(), attachments: z.array(AttachmentSchema).optional() });
