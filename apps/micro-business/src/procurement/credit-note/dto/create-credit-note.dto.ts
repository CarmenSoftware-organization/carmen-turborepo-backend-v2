...
// Re-export validate functions for use with CreateCreditNote
export {
  validateVendorIdExists,
  validateVendorIdsExist,
  validateCurrencyIdExists,
...
/**
 * Create CreateCreditNoteSchema with async database validation
 */
export function createCreditNoteCreateValidation(prisma: PrismaClient) {
  return CreateCreditNoteSchema.superRefine(async (data, ctx) => {
    // Validate vendor_id
    if (data.vendor_id) {
